...
setwd("C:/Users/nj35/OneDrive - University of St Andrews/MRes/Ethovision R-Analysis")
getwd()
```
...
```{r}
# Load Excel file (.xlsx) — specify sheet if needed
...
#ggsave("oft_graph(distance).svg", plot = plot_oft, width = 8, height = 6)
```
...
#ggsave("oft_graph(speed).svg", plot = plot_oft2, width = 8, height = 6)
```
...
#ggsave("oft_graph(peri).svg", plot = plot_oft3, width = 8, height = 6)
```
...
#ggsave("nor_graph(contact).svg", plot = plot_nor2, width = 8, height = 6)
```
...
#ggsave("epm_graph(freq).svg", plot = plot_epm, width = 8, height = 6)
```
...
#ggsave("oft_cloud_graph_SE(distance).svg", plot = plot_raincloud_oft, width = 8, height = 6)
```
...
#ymin:	Lower whisker = max(min value, Q1 - 1.5 × IQR).
#lower:	Q1 (25th percentile).
#middle:	Median (50th percentile).
#upper:	Q3 (75th percentile).
#ymax:	Upper whisker = min(max value, Q3 + 1.5 × IQR).
...
#ggsave("oft_cloud_graph_SE(speed).svg", plot = plot_raincloud_oft, width = 8, height = 6)
```
...
#ggsave("oft_cloud_graph_SE(peri).svg", plot = plot_raincloud_oft3, width = 8, height = 6)
```
...
#ggsave("nor_cloud_graph_SE(memory).svg", plot = plot_raincloud_nor, width = 8, height = 6)
```
...
#ggsave("epm_cloud_graph_SE(freq_c).svg", plot = plot_raincloud_epm, width = 8, height = 6)
```
...
#ggsave("epm_cloud_graph_SE(time_c.svg", plot = plot_raincloud_epm2, width = 8, height = 6)
```
...
#ggsave("3ct_cloud_graph_SE(general).svg", plot = plot_raincloud_3ct, width = 8, height = 6)
```
...
#ggsave("3ct_cloud_graph_SE(novel).svg", plot = plot_raincloud_3ct2, width = 8, height = 6)
```
...
#ggsave("ldct_cloud_graph_SE.svg", plot = plot_raincloud_ldct, width = 8, height = 6)
```
