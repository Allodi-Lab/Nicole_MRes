...
```{r}
txt1 <- capture.output(summary(pairwise_oft))

pdf("pairwise_oft_summary.pdf", width = 8.27, height = 11.69)  # A4

lines_per_page <- 50
n_pages <- ceiling(length(txt1) / lines_per_page)

for (i in 1:n_pages) {
  plot.new()
  
  start <- (i - 1) * lines_per_page + 1
  end <- min(i * lines_per_page, length(txt1))
  
  page_text <- txt1[start:end]
  
  text(0, 1, paste(page_text, collapse = "\n"),
...
```{r}
#for ilary
df_oft_ilary <- as.data.frame(summary(pairwise_oft), stringsAsFactors = FALSE)

# Filter for significant p-values 
df_oft_ilary_sig <- df_oft_ilary[
  as.numeric(gsub("<", "", df_oft_ilary$p.value)) < 0.05,
]
...
```{r}
txt2 <- capture.output(summary(pairwise_nor))

pdf("pairwise_nor_summary.pdf", width = 8.27, height = 11.69)  # A4

lines_per_page <- 50
n_pages <- ceiling(length(txt4) / lines_per_page)

for (i in 1:n_pages) {
  plot.new()
  
  start <- (i - 1) * lines_per_page + 1
  end <- min(i * lines_per_page, length(txt2))
  
  page_text <- txt2[start:end]
...
```{r}
txt3 <- capture.output(summary(pairwise_epm))

pdf("pairwise_epm_summary.pdf", width = 8.27, height = 11.69)  # A4

lines_per_page <- 50
n_pages <- ceiling(length(txt3) / lines_per_page)

for (i in 1:n_pages) {
  plot.new()
  
  start <- (i - 1) * lines_per_page + 1
  end <- min(i * lines_per_page, length(txt3))
  
  page_text <- txt3[start:end]
...
```{r}
txt4 <- capture.output(summary(pairwise_3ct))

pdf("pairwise_3ct_summary.pdf", width = 8.27, height = 11.69)  # A4
...
for (i in 1:n_pages) {
  ...
  start <- (i - 1) * lines_per_page + 1
  end <- min(i * lines_per_page, length(txt4))
  
  page_text <- txt4[start:end]
